...
def bootstrap_toolkit(root_path):
    ...
    MaxPlus.StatusPanel.DisplayTempPrompt("Loading Flow Production Tracking integration...", 5000)
...
def progress_callback(progress_value, message):
    ...
    MaxPlus.StatusPanel.DisplayTempPrompt("Flow Production Tracking: %s" % message, 2000)
...
def handle_bootstrap_completed(engine):
    ...
    if PluginProperties.running_as_standalone_plugin:
        engine.register_command("Log Out of Flow Production Tracking", _on_logout, {"type": "context_menu"})
...
def shutdown_toolkit():
    ...
    if engine:
        logger.info("Stopping the Flow Production Tracking engine.")
        ...
        logger.debug("The Flow Production Tracking engine was already stopped!")
...
def _login_user():
    ...
    try:
        ...
        sgtk_logger.info("Flow Production Tracking login was cancelled by the user.")
        return
...
def _create_login_menu():
    ...
    login_action = MaxPlus.ActionFactory.Create(
        constants.SG_MENU_ITEMS_CATEGORY, "Log In to Flow Production Tracking...", _login_user)
    mb.AddItem(login_action)
    mb.AddSeparator()

    jump_to_website_action = MaxPlus.ActionFactory.Create(
        constants.SG_MENU_ITEMS_CATEGORY, "Learn about Flow Production Tracking...", _jump_to_website)
    mb.AddItem(jump_to_website_action)

    jump_to_signup_action = MaxPlus.ActionFactory.Create(
        constants.SG_MENU_ITEMS_CATEGORY, "Try Flow Production Tracking for Free...", _jump_to_signup)
